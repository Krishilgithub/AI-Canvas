      ...
      >
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage content pipeline, analyze trends, and auto-schedule posts.
          </p>
        </div>
        <div
...
function TrendsView() {
  ...
  useEffect(() => {
    setLoading(true);
    fetcher(
      `/trends?page=${page}&limit=10&category=${category}&platform=linkedin`,
    )
      .then((res) => {
        ...
        <Card className="bg-gradient-to-br from-primary/5 to-transparent border-primary/10">
          ...
          <CardContent>
            <div className="text-xs text-muted-foreground">
              Top 3 trends align with your niche today.
            </div>
          </CardContent>
        </Card>
        <Card>
        ...
        <CardHeader className="pb-2">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>
                Topics currently gaining velocity in your niche.
              </CardDescription>
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  toast.promise(
                    poster("/scan", { platform: "linkedin" }).then(() =>
                      window.location.reload(),
                    ),
                    {
                      loading: "Scanning trends...",
                      success: "Trends scanned!",
                      error: "Failed to scan trends",
                    },
                  );
                }}
              >
                <Sparkles className="mr-2 h-4 w-4" /> Scan Trends
              </Button>
            </div>
          </div>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Log of all actions taken by the AI agent.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {[
            {
              action: "Published 'SaaS Trends 2025'",
              time: "2 hours ago",
              status: "Success",
            },
            {
              action: "Generated 3 drafts for topic 'AI Marketing'",
              time: "5 hours ago",
              status: "Success",
            },
            {
              action: "Failed to connect to image generator",
              time: "Yesterday",
              status: "Error",
            },
            {
              action: "Trend scan completed (14 new topics)",
              time: "Yesterday",
              status: "Success",
            },
          ].map((log, i) => (
