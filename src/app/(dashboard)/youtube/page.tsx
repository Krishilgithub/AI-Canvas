...
import {
  ...
  Wifi, WifiOff, Activity, Filter, Video
} from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500" suppressHydrationWarning>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-border/50">
        <div className="flex items-center gap-4">
          <div className="h-11 w-11 rounded-xl bg-red-600/10 flex items-center justify-center text-red-600 shrink-0">
            <YouTubeIcon className="h-5 w-5" />
          </div>
          <div>
            <h1 className="text-2xl font-bold font-heading tracking-tight">YouTube Studio</h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Automated Shorts · long-form ideas · trend scaling
            </p>
          </div>
        </div>
      ...
      {activeTab === "approval"  && <div className="h-[calc(100vh-220px)] overflow-hidden"><ContentApproval platform="youtube" /></div>}
      {activeTab === "config"    && <YouTubeConfigurationPanel />}
      {activeTab === "logs"      && <HistoryView platform="youtube" />}
    </div>
  );
}
...
function TrendsView() {
  ...
  const CATEGORIES = ["All", "Education", "Entertainment", "Tech", "Life Hacks"];

  const loadTrends = useCallback(() => {
    setLoading(true);
    fetcher(`/trends?page=${page}&limit=10&category=${category}&platform=youtube`)
      .then((res) => {
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      fetcher("/posts?status=needs_approval&platform=youtube&limit=1"),
      fetcher("/connections"),
      ...
      if (connRes.status === "fulfilled") {
        ...
        setConnection(conns.some((c) => c.platform === "youtube") ? "connected" : "disconnected");
      }
    });
  }, []);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      stager("analyzing");
      await poster("/scan", { platform: "youtube" });
      stager("drafting");
      await new Promise((r) => setTimeout(r, 500));
      stager("done");
      loadTrends();
      toast.success("Trends scanned and scripts queued for approval!");
    } catch (err: any) {
  ...
  const handleDraft = (trend: Trend) => {
    toast.promise(
      poster("/create-draft", { trend_id: trend.id, platform: "youtube" }),
      {
        loading: "Structuring script…",
        success: 'Script drafted! Check "Review Queue".',
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="pt-5 pb-4">
            ...
            {highImpactCount > 0 && (
              <p className="text-xs text-green-600 mt-2 font-medium">{highImpactCount} high-viral potential</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className={cn(pendingCount > 0 && "border-amber-500/30 bg-amber-500/5")}>
          <CardContent className="pt-5 pb-4">
            ...
            {pendingCount > 0 && (
              <p className="text-xs text-amber-600 mt-2 font-medium">{pendingCount} script{pendingCount > 1 ? "s" : ""} need approval</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-5 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">Monthly Uploads</p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              ...
              <CardDescription className="mt-1">
                Topics currently gaining video velocity — ranked by AI impact score.
              </CardDescription>
        ...
        <CardContent className="pt-0">
          {loading ? (
            ...
            <EmptyState
              ...
              description="Click 'Scan Trends' to let the AI analyze today's video ranking metrics."
              action={
                <Button onClick={handleScan} className="gap-2 bg-red-600 hover:bg-red-700 text-white">
            ...
            <>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-muted-foreground bg-secondary/30 text-xs font-semibold uppercase tracking-wide">
                    <tr>
                      ...
                      <th className="px-5 py-3">Viral Potential</th>
                  ...
                  <tbody className="divide-y divide-border/40 bg-card">
                    {trends.map((row) => (
                      <tr
                        key={row.id}
                        className="group hover:bg-red-600/5 transition-colors"
                      >
                        ...
                        <td className="px-5 py-4 text-right">
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                            ...
                            <Button
                              size="sm"
                              className="h-7 px-3 text-xs gap-1.5 bg-red-600 hover:bg-red-700 text-white"
                              onClick={() => handleDraft(row)}
                            >
                              <Zap className="h-3 w-3" /> Script
                            </Button>
                          </div>
                        </td>
                      </tr>
              ...
              <div className="flex items-center justify-between pt-4">
                <p className="text-xs text-muted-foreground">
                  Page {page} of {totalPages || 1} · {trends.length} concepts
                  {topScore > 0 && <> · Top potential: <span className="font-semibold text-foreground">{topScore}/100</span></>}
                </p>
