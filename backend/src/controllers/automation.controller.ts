...
export class AutomationController {
    ...
    try {
      ...
      for (const trend of savedTrends) {
        if (trend.velocity_score > 70) {
          ...
          try {
            let draftContent = await workflowService.generatePost(
  ...
  generateManualPost = async (req: AuthRequest, res: Response) => {
    try {
      ...
      let content = await workflowService.generatePost(
        {
          topic,
  ...
  // ────────────────────────────────────────────────────────────────────────
  remixPost = async (req: AuthRequest, res: Response) => {
    try {
      const user_id = req.user?.id;
      if (!user_id) return res.status(401).json({ error: "Unauthorized" });
      ...
      const content = await workflowService.generatePost(
        ...
        userProfile || undefined,
        "Remix and translate this content precisely for the target platform's formatting and style length."
      );
