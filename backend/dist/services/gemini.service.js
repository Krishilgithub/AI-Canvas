...
exports.geminiService = exports.GeminiService = exports.PLATFORM_CHAR_LIMITS = void 0;
...
class GeminiService {
    getModel(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (userId) {
                const userKey = yield keys_service_1.keysService.getKey(userId, 'gemini');
                if (userKey)
                    apiKey = userKey;
            }
            if (!apiKey)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    analyzeTrendIntelligence(input) {
        return __awaiter(this, void 0, void 0, function* () {
            const { userId, genre, keywords, target_platform, time_window = "last 24 hours", platform_data, max_trends = 10, // FIX: was hardcoded to 5 — now configurable, default 10
             } = input;
...
If the input data is too weak, noisy, or unrelated to the user's genre, return an empty trends array rather than fabricating results.`;
            try {
                ...
                return parsed.trends.map((t) => (Object.assign(Object.assign({}, t), { velocity_score: Math.round(t.impact_score * 100), insight: t.reasoning, title: t.topic, category: genre })));
            }
            catch (error) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    analyzeTrendPotential(articles, userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const model = yield this.getModel(userId);
...
${articles.map((a, i) => `${i}: ${a.title}`).join("\n")}`;
                ...
                return articles.map((article, index) => {
                    const insight = analysis.find((a) => a.index === index);
                    return Object.assign(Object.assign({}, article), { topic: article.title, summary: article.description || "", platforms: ["linkedin"], impact_score: insight ? insight.velocity_score / 100 : 0, confidence: 0.7, engagement_signals: { likes: 0, comments: 0, shares: 0, velocity: 0 }, reasoning: (insight === null || insight === void 0 ? void 0 : insight.reason) || "No analysis available", suggested_angle: (insight === null || insight === void 0 ? void 0 : insight.suggested_angle) || "", velocity_score: insight ? insight.velocity_score : 0, insight: insight ? insight.reason : "No analysis available", title: article.title, category: article.category || "General" });
                });
            }
            catch (error) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    buildUnscoredFallback(articles, genre) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    generateDraft(topic_1, context_1, userContext_1, userId_1) {
        return __awaiter(this, arguments, void 0, function* (topic, context, userContext, userId, platform = "linkedin") {
            const model = yield this.getModel(userId);
            ...
            const charLimit = exports.PLATFORM_CHAR_LIMITS[platform.toLowerCase()] || 3000;
...
Output ONLY the post text. No meta-commentary, no "Here is your post:", no quotes around the output.`;
    ...
    rewriteText(text_1, instruction_1, userId_1) {
        return __awaiter(this, arguments, void 0, function* (text, instruction, userId, platform = "linkedin") {
            const model = yield this.getModel(userId);
            if (!model) {
                throw new Error("AI_UNAVAILABLE");
            }
...
function getPlatformFormatRules(platform) {
...
function enforcePlatformCharLimit(content, platform) {
    const limit = exports.PLATFORM_CHAR_LIMITS[platform.toLowerCase()];
    if (!limit)
        return content;
    ...
    console.warn(`[GeminiService] Draft for ${platform} is ${content.length} chars, exceeds soft limit of ${limit}. ` +
